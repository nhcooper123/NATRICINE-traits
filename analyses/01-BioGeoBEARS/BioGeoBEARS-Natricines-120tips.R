#Biogeobears codes used in Deepak et al 2021.
...
#Set directory to BIOGEOBEARS folder
extdata_dir = np(system.file("extdata", package="BioGeoBEARS"))
extdata_dir 
list.files(extdata_dir)

#import tree
trfn = "/data/natricine.newick.tre"
trfn


trfn = np(paste(addslash(extdata_dir), "120tip.newick", sep=""))
moref(trfn)
...
# two geography files for the two datasets used in this study

# geography file for 8 areas

geogfn = np(paste(addslash(extdata_dir), "Totnat-geo-data-8areas", sep=""))

# geography file for 10 areas
#geogfn = np(paste(addslash(extdata_dir), "Totnat-geo-data-120", sep=""))
...
# For the second analysis change number of areas to 10. everything else remains the same
numstates_from_numareas(numareas=8, maxareas=3, include_null_range=TRUE)


#Codes modified from ---http://phylo.wikidot.com/biogeobears---
#Note comparisions are made for all six models but we do not use BAYAREALIKE and BAYAREALIKE+J models 
# Run DEC
#######################################################
...
runslow = TRUE
resfn = "Totnat_DEC_M0_unconstrained_v1.Rdata"
if (runslow)
{
...
#######################################################
analysis_titletxt ="BioGeoBEARS DEC on Totnat M0_unconstrained"
...
# States
res1 = plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="text", label.offset=0.45, tipcex=0.7, statecex=0.6, splitcex=0.6, titlecex=0.7, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)

# Pie chart
plot_BioGeoBEARS_results(results_object, colors_list_for_states, analysis_titletxt, addl_params=list("j"), plotwhat="pie", label.offset=0.45, tipcex=0.2, statecex=0.2, splitcex=0.2, titlecex=0.2, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges, colors_list_for_states=colors_list_for_states)
...
#########################################################################
#########################################################################
colors_legend("Totnat-geo-data-8areas",
              colors_list_for_states, legend_ncol = NULL,
              legend_cex = 1)

colors_list_for_states  = c("white", "red", "green", "blue", "pink", "orange","yellow","cyan")
#######################################################
# Run DEC+J
#######################################################
BioGeoBEARS_run_object = define_BioGeoBEARS_run()
...
resfn = "nat_DEC+J_M0_unconstrained_v1.Rdata"
runslow = TRUE
if (runslow)
{
...
# PDF plots
#######################################################
pdffn = "nat_DEC_vs_DEC+J_M0_unconstrained_v1.pdf"
pdf(pdffn, width=6, height=6)

#######################################################
# Plot ancestral states - DEC
#######################################################
analysis_titletxt ="BioGeoBEARS DEC on nat M0_unconstrained +J"
...
plot_BioGeoBEARS_results(results_object, analysis_titletxt, addl_params=list("j"), plotwhat="pie", label.offset=0.5, tipcex=0.25, statecex=0.25, splitcex=0.0, titlecex=0.8, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)

#######################################################
# Plot ancestral states - DECJ
#######################################################
analysis_titletxt ="BioGeoBEARS DEC+J on Totnat M0_unconstrained"
...
runslow = TRUE
resfn = "nat_DIVALIKE_M0_unconstrained_v1.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
resfn = "nat_DIVALIKE+J_M0_unconstrained_v1.Rdata"
runslow = TRUE
if (runslow)
{
  #sourceall("/Dropbox/_njm/__packages/BioGeoBEARS_setup/")
...
pdffn = "nat_DIVALIKE_vs_DIVALIKE+J_M0_unconstrained_v1.pdf"
pdf(pdffn, width=6, height=6)

#######################################################
# Plot ancestral states - DIVALIKE
#######################################################
analysis_titletxt ="BioGeoBEARS DIVALIKE on nat M0_unconstrained"
...
#######################################################
analysis_titletxt ="BioGeoBEARS DIVALIKE+J on nat M0_unconstrained"
...
runslow = TRUE
resfn = "nat_BAYAREALIKE_M0_unconstrained_v1.Rdata"
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
  res    
...
resfn = "nat_BAYAREALIKE+J_M0_unconstrained_v1.Rdata"
runslow = TRUE
if (runslow)
{
  res = bears_optim_run(BioGeoBEARS_run_object)
...
pdffn = "nat_BAYAREALIKE_vs_BAYAREALIKE+J_M0_unconstrained_v1.pdf"
pdf(pdffn, width=6, height=6)

#######################################################
# Plot ancestral states - BAYAREALIKE
#######################################################
analysis_titletxt ="BioGeoBEARS BAYAREALIKE on nat M0_unconstrained"
...
#######################################################
analysis_titletxt ="BioGeoBEARS BAYAREALIKE+J on nat M0_unconstrained"
